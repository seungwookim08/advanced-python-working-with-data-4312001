...
print(
    any(
        quake["properties"]["felt"] is not None and quake["properties"]["felt"] > 25000
...
print(
    sum(
        quake["properties"]["felt"] is not None and quake["properties"]["felt"] >= 500
        for quake in data["features"]
    )
)
...
print(
    sum(
        quake["properties"]["mag"] is not None and quake["properties"]["mag"] >= 6
        for quake in data["features"]
    )
)
